...
xlsx файл на 12000 строк ~10.45 MB:
- ~1.25 сек
- ~175MB RAM
- ~1.02GB было выделено и освобождено за время запроса (характеризует “объём работы” GC)
- ~248MB RAM удерживаемой рантаймом Go у ОС после запроса (Sys)
- На ~10MB файл нужно закладывать ≈250–300MB RAM (с запасом) и ~1–1.5 секунды CPU‑времени на ядро.
